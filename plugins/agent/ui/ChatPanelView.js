export class ChatPanelView {
  ...
  mount(mountNode) {
    ...
    const mascotUrl = configAssetUrl(this.config.branding?.mascotAsset);
    const sttOk = !!this.voiceSupported.stt;
    const ttsOk = !!this.voiceSupported.tts;

    this.root.innerHTML = `
      <button class="netty-agent-toggle" type="button" aria-label="Open Netty Brains assistant" aria-expanded="false">
        <div class="netty-agent-mascot netty-agent-bob" data-role="toggle-mascot" aria-hidden="true">
          <img src="${escapeAttr(mascotUrl)}" alt="">
        </div>
      </button>

      <div class="netty-agent-widget" role="dialog" aria-label="Netty Brains chat widget" aria-hidden="true">
        <div class="netty-agent-header">
          ...
          <div class="netty-agent-header-text">
            <div class="netty-agent-title">Netty Brains</div>
  ...
  _setOpen(open) {
    ...
    if (this.toggleBtn) {
      ...
      this.toggleBtn.setAttribute(
        "aria-label",
        this._open ? "Close Netty Brains assistant" : "Open Netty Brains assistant"
      );
...
function configAssetUrl(path) {
  if (!path) return "";
  if (path.startsWith("http://") || path.startsWith("https://")) return path;
  if (path.startsWith("./")) return "/plugins/agent/" + path.slice(2);
  if (path.startsWith("/")) return path;
  return "/plugins/agent/" + path;
}
